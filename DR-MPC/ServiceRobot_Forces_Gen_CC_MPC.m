...
for k=2:model.N
    ...
    model.hl{k} = [zeros(1,L), ones(1,Nobs)];
    model.hu{k} = [inf(1,L), inf(1,Nobs)];
end
...
global nu nx ny Nn L w b W veh_rad alpha delta Nobs
...
%     rho = z(nu+nx+1:nu+nx+4);
    for l=1:L
        ...
        Sigma_hat{l} = [Sigma_hat{l}(1) Sigma_hat{l}(2);Sigma_hat{l}(2) Sigma_hat{l}(4)];
        posio{l} = (x - mu_hat{l});
        a{l} = sqrt(transpose(posio{l})*posio{l});
        f = [f; a{l} - (W+veh_rad)^2 - delta*sqrt(2*transpose(posio{l})*Sigma_hat{l}*posio{l})/a{l}];
   end
    for i=1:Nobs
