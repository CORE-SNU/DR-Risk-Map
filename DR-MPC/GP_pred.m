if t==1
    xtr1={zeros(1,2),zeros(1,2),zeros(1,2),zeros(1,2)};
    ytra1={normrnd(0,0.0001),normrnd(0,0.0001),normrnd(0,0.0001),normrnd(0,0.0001)};
    ytrr1={normrnd(0,0.0001),normrnd(0,0.0001),normrnd(0,0.0001),normrnd(0,0.0001)};
end

for kk=1:size(obs_state,3)
        xt_mu1=obs_state(1:2,t,kk);
        xt_var1=zeros(2);
...
%         thetavar1(1)=0; 
        ...
        for k=1:K
            ...
            [xt_mu1(:,k+1), x_mud1, u_mud1]=vehicledyn1([amu1(k) rmu1(k)],xt_mu1(:,k)');
...
% %             end
            ...
            xy21_var_inv{t}(:,:,k,kk)=inv(diag([xt_var1(1,1,k) xt_var1(2,2,k)]));
...
% % % %                     set(movv1{kk}(k),'ydata',movobs11{kk}(:,2,k));
                            set(movv1{kk}(k),'Position',[xy21{t}(1,k,kk)-W xy21{t}(2,k,kk)-W 2*W 2*W]);
                catch
% % % %                     movv1{kk}(k)=fill(movobs11{kk}(:,1,k),movobs11{kk}(:,2,k),[0 .5 .5],'EdgeColor',[0 .5 .5],'FaceAlpha',0.05);
                            movv1{kk}(k)=rectangle('Position',[xy21{t}(1,k,kk)-W xy21{t}(2,k,kk)-W 2*W 2*W],'FaceColor',[0 .5 .5 0.05],'EdgeColor',[0 .5 .5],'Curvature',[1 1]);
                end
            end
        end     
        try
...
% % % %             set(movv1{kk}(1),'ydata',movobs11{kk}(:,2,1));
            set(movv1{kk}(1),'Position',[xy21{t}(1,1,kk)-W xy21{t}(2,1,kk)-W 2*W 2*W]);
        catch
            movv1{kk}(1)=rectangle('Position',[xy21{t}(1,1,kk)-W xy21{t}(2,1,kk)-W 2*W 2*W],'FaceColor',[0 .5 .5 1],'EdgeColor',[0 .5 .5],'Curvature',[1 1]);
        end
% % % % %         if t~=1
% % % % %         set(obsplot{kk},'xdata',obs_state(1,t:t+K,kk),'ydata',obs_state(2,t:t+K,kk));
% % % % %         else
% % % % %             obsplot{kk}=plot(obs_state(1,t:t+K,kk),obs_state(2,t:t+K,kk),'*-r');
% % % % %         end
        drawnow
end
