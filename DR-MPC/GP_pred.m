...
% if t==1
%     x_tr = {zeros(1,2),zeros(1,2),zeros(1,2),zeros(1,2)};
%     for j=1:obs_nu
%         y_tr{j} = {normrnd(0,0.0001),normrnd(0,0.0001),normrnd(0,0.0001),normrnd(0,0.0001)};
%     end
% end
%% Initialize GP datasets 
if t==1
...
for obs_idx=1:size(obs_state,3) 
        ...
        for k=1:K
            ...
            [xt_mu(:,k+1), dmu_x_dx, dmu_x_du] = obs_dyn(ut_mu(k,:),xt_mu(:,k)');
...
% Leaves only the latest M_max observations
