% if t~=1 && size(nodes_to_grow,2)<=1
%     nodes_to_grow=nodes([nodes.index]==root.index); 
% else
...
while ~isempty(node_stack)
    ...
    if ~isempty(current_node.sons) && current_node.depth<=K && current_node.isFree
        ...
        if nodes(index).depth~=0
            nodes(index).cost=nodes([nodes.index]==nodes(index).parent).cost+w*nodes(index).risk+trajLength(nodes([nodes.index]==nodes(index).parent).coord,nodes(index).coord);
        end
