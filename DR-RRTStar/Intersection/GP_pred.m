...
%             u_var1(:,:,k)=diag([avar1(k) rvar1(k)]);
            xvvar1(:,:,k)=xt_var1(:,:,k)*a_mud1(k,:)';
            [xt_mu1(:,k+1), x_mud1, u_mud1]=vehicledyn(amu1(k),[xt_mu1(:,k)' obs_state(3,t,kk)]);
%             xt_var1(:,:,k+1)=x_mud1*xt_var1(:,:,k)*x_mud1'+u_mud1*u_var1(:,:,k)*u_mud1'+x_mud1*xvvar1(:,:,k)*u_mud1'+u_mud1*xvvar1(:,:,k)'*x_mud1';
            xt_var1(:,:,k+1)=x_mud1*xt_var1(:,:,k)*x_mud1'+u_mud1*avar1(k)*u_mud1';
%             try
...
% %             end
            xy21_var{t}{kk}(:,:,k)=diag([sqrt(xt_var1(1,1,k)) sqrt(xt_var1(2,2,k))]);
...
% % % %                     set(movv1{kk}(k),'ydata',movobs11{kk}(:,2,k));
                            set(movv1{kk}(k),'Position',[xy21{t}{kk}(k,1)-W/2 xy21{t}{kk}(k,2)-W/2 W W]);
                catch
% % % %                     movv1{kk}(k)=fill(movobs11{kk}(:,1,k),movobs11{kk}(:,2,k),[0 .5 .5],'EdgeColor',[0 .5 .5],'FaceAlpha',0.05);
                            movv1{kk}(k)=rectangle('Position',[xy21{t}{kk}(k,1)-W/2 xy21{t}{kk}(k,2)-W/2 W W],'FaceColor',[0 .5 .5 0.05],'EdgeColor',[0 .5 .5],'Curvature',[1 1]);
                end
            end
        end     
        try
...
% % % %             set(movv1{kk}(1),'ydata',movobs11{kk}(:,2,1));
            set(movv1{kk}(1),'Position',[xy21{t}{kk}(1,1)-W/2 xy21{t}{kk}(1,2)-W/2 W W]);
        catch
            movv1{kk}(1)=rectangle('Position',[xy21{t}{kk}(1,1)-W/2 xy21{t}{kk}(1,2)-W/2 W W],'FaceColor',[0 .5 .5 1],'EdgeColor',[0 .5 .5],'Curvature',[1 1]);
        end
        drawnow
    end