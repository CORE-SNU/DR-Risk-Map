function distance_from_goal=trajLength(pose,goal)
    global root;
    pose_euclide_distance=vecnorm(pose(1:2,:)-goal(1:2));
    root_euclide_distance=vecnorm(root.coord(1:2)-goal(1:2));
    position_improvement = pose_euclide_distance./root_euclide_distance;
    rotation_diff = atan2(goal(2)-pose(2,:),goal(1)-pose(1,:))-pose(3,:);
    rotation_diff(rotation_diff > pi) = rotation_diff(rotation_diff > pi) - 2*pi;
    rotation_diff(rotation_diff < -pi) = rotation_diff(rotation_diff < -pi) + 2*pi;
